...
void testUniform()
{
    ...
    std::array<std::size_t, N> xo;
    xo.fill(0);

    //error seed
    std::array<std::uint64_t, 2> s1 { 3166475690, 2492647370 }; 
    std::array<std::uint64_t, 2> s2 { 1685024553, 3015549104 }; 

    RandomState seed1(s1.data());
    RandomState seed2(s2.data());
   
    std::cout << seed1.m_state[0] << ", " << seed1.m_state[1]<<std::endl;
    std::cout << seed2.m_state[0] << ", " << seed2.m_state[1] << std::endl;
 
    const auto start = std::chrono::high_resolution_clock::now();
    for (std::size_t i = 0; i < S; ++i) {
        const auto t = seed2.randomUniform(N);
        ++xo[t];
    }
    const auto stamp = std::chrono::high_resolution_clock::now();
    for (std::size_t i = 0; i < S; ++i) {
        const auto t = static_cast<std::size_t>(randomUniform<double>(seed1) * N);
        ++pcg[t];
    }

    const auto end = std::chrono::high_resolution_clock::now();
    
    std::cout << "PCG, shiro" << std::endl;
    for (std::size_t i = 0; i < N; ++i) {
        std::cout << pcg[i] <<", " << xo[i] << std::endl;
    }

    auto pcg_time = std::chrono::duration_cast<std::chrono::milliseconds> (stamp - start);
    auto xo_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - stamp);

    std::cout << "PCG time: " << pcg_time.count() << std::endl;
    std::cout << "Xo time:  " << xo_time.count() << std::endl;
...
void testWeights()
{
    ...
    std::cout << seed.m_state[0] << " " << seed.m_state[1] << " " << r1 << " " << r2 << std::endl;
