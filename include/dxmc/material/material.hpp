...
public:
    ...
    {
        return momentumTransferMax(energy) * std::sin(angle * T { 0.5 }); // per �
    }

    static T momentumTransferCosAngle(T energy, T cosAngle)
    {
        return momentumTransferMax(energy) * std::sqrt((1 - cosAngle) * T { 0.5 }); // per �
    }

    static constexpr T momentumTransferMax(T energy)
    {
        static constexpr T hc_si = 1.239841193E-6; // ev*m
        static constexpr T m2A = 1E10; // meters to �ngstr�m
        static constexpr T eV2keV = 1E-3; // eV to keV
        static constexpr T hc = hc_si * m2A * eV2keV; // kev*�
