...
concept StaticKDTreeType = requires(U u, Particle<T> p, std::array<T, 3> vec) {
                               ...
                               {
                                   u.intersect(p)
                                   } -> std::same_as<std::optional<typename T>>;

                               {
                                   u.center()
                                   } -> std::same_as<std::array<typename T, 3>>;

                               {
                                   u.AABB()
                                   } -> std::same_as<std::array<typename T, 6>>;
                           };
...
class StaticKDTree {
public:
    StaticKDTree(unsigned int max_depth = 5)
        : m_maxDepth(max_depth) {};
    
    template<same_as_any<Us...> U>
    StaticKDTree(std::vector<U>& triangles, unsigned int max_depth = 5)
        : m_maxDepth(max_depth) {}


    template <same_as_any<Us...> U>
    void insert(U value) {
        std::get<std::vector<U>>(m_triangles).push_back(value);
    }
    void buildTree()
    {
        if (triangles.size() == 0)
...
private:
    ...
    T m_plane = 0;
    //std::vector<U> m_triangles;
    std::tuple<std::vector<Us>...> m_triangles;

    std::optional<StaticKDTree<T, Us...>> m_left = std::nullopt;
    std::optional<StaticKDTree<T, Us...>> m_right = std::nullopt;
    //std::unique_ptr<KDTree<U>> m_left = nullptr;
    //std::unique_ptr<KDTree<U>> m_right = nullptr;
    friend class StaticKDTree<T, Us...>;
};
