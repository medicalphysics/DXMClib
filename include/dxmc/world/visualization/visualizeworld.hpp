...
public:
    ...
    void addColorByValueItem(WorldItemBase<T>* item)
    {
        m_colorByValue.insert(item);
    }
    void setColorByValueMinMax(T min, T max)
    {
        ...
        if constexpr (std::is_same<U, T>::value) {
            if (m_colorByValue.size() > 0) {
                if (m_colorByValueClamp[0] >= m_colorByValueClamp[1]) {
                    ...
                    const T div = max > 1 ? 255 / (max - 1) : 1;
                    for (std::size_t i = 0; i < buffer.size(); i = i + 4) {
                        if (buffer[i] > 1) {
                            const auto ind = static_cast<std::uint8_t>((buffer[i] - 1) * div);
                            const auto c = turboColorMap<T>()[ind];
                    ...
                    for (std::size_t i = 0; i < buffer.size(); i = i + 4) {
                        if (buffer[i] > 1) {
                            ...
                            const auto ind = static_cast<std::uint8_t>(255 * std::min(val, T { 1 }));
                            const auto c = turboColorMap<T>()[ind];
                            for (std::size_t j = 0; j < 3; ++j)
                                buffer[i + j] = c[j];
                            buffer[i + 3] = 1;
...
protected:
    ...
    std::array<U, 3> colorOfItem(const Particle<T>& p, const std::array<T, 3>& normal, const WorldItemBase<T>* item) const
    {
        if (auto search = m_colorIndex.find(item); search != m_colorIndex.end()) {
            const auto scaling = T { 0.8 } + T { 0.2 } * vectormath::dot(p.dir, normal);
...
private:
    ...
    std::set<const WorldItemBase<T>*> m_colorByValue;
    std::array<T, 2> m_colorByValueClamp = { 0, 1 };
};
}